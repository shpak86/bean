...
func TestNewRulesScoreCalculator_InitRuleError(t *testing.T) {
	// CEL expression with undefined variable if the environment doesn't declare it
	script := `
- when: "unknownField == 1"
  then:
    behavior: 0.5
...
`
	...
	// Add trace that satisfies the condition
	repo.Append("user1", trace.Trace{"mouseMoves": int32(10)})
...
`
	...
	// Add trace that does NOT satisfy the condition
...
`
	...
	// Two traces: each will match both rules → should be clamped to 1.0
	repo.Append("user1", trace.Trace{"mouseMoves": int32(10)})
	repo.Append("user1", trace.Trace{"mouseMoves": int32(10)})
...
func TestNewRulesScoreCalculator_ValidYAMLInvalidStructure(t *testing.T) {
	// Valid YAML, but doesn't match []Rule
